...
export function SupportTicketManagement({ userRole }: SupportTicketManagementProps) {
                            ...
                            {tickets.map((ticket) => (
                                <div key={ticket.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50">
                                    ...
                                    <div className="text-right">
                                        ...
                                        <Button size="sm" variant="outline" className="mt-2" onClick={() => setSelectedTicket(ticket)}>
...
function TicketDetailsModal({ ticket, onClose }: { ticket: SupportTicket, onClose: () => void }) {
                ...
                <CardContent className="flex-1 overflow-y-auto p-6 space-y-6 bg-gray-50">
                    <div className="bg-white p-4 rounded-lg shadow-sm border">
                        <div className="flex justify-between mb-2">
                            <span className="font-semibold text-gray-900">{ticket.userName || ticket.userEmail}</span>
                    ...
                    {loading ? (
                        ...
                        messages.map((msg) => (
                            <div key={msg.id} className={`p-4 rounded-lg shadow-sm border $$'{msg.isInternal ? 'bg-yellow-50 border-yellow-200 ml-12' : 'bg-blue-50 border-blue-200 ml-12'}'}`}>
                                <div className="flex justify-between mb-2">
                                    <span className="font-semibold flex items-center gap-2">
                                        {msg.userName || msg.userEmail}
                                        {msg.isInternal && <Badge variant="outline" className="text-xs bg-yellow-100 text-yellow-800">Internal Note</Badge>}
                                    </span>
                ...
                <div className="p-4 border-t bg-white">
                    <div className="flex items-center gap-4 mb-2">
                        <label className="flex items-center gap-2 text-sm text-gray-600">
                            <input type="checkbox" checked={isInternal} onChange={(e) => setIsInternal(e.target.checked)} className="rounded text-blue-600" />
                    ...
                    <div className="flex gap-2">
                        ...
                        />
                        <Button className="h-auto px-6 whitespace-nowrap" onClick={handleSendReply} disabled={sending || !replyMessage.trim()}>
