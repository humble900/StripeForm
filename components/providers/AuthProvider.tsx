...
import { getDeviceFingerprint, createAnonymousUserData } from '@/lib/fingerprint'
import { userTracking as userTrackingManager, UserTrackingData } from '@/lib/user-tracking'
...
const migrateAnonymousToAuthenticated = async (fingerprint: string, userId: string) => {
...
interface AuthContextType extends AuthState {
  ...
  getUserTrackingData: () => Promise<{ ip: string; fingerprint: string; cookies: string[] }>
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
          ...
          try {
            ...
            if (userData) {
              ...
              const frontendUser: User = {
                ...
                name: userData.firstName && userData.lastName ? `${userData.firstName} ${userData.lastName}` : userData.firstName || userData.lastName || undefined,
                ...
                subscription_status: userData.subscriptionStatus as 'active' | 'inactive' | 'cancelled',
                ...
                created_at: userData.createdAt instanceof Date ? userData.createdAt.toISOString() : new Date(userData.createdAt).toISOString(),
                updated_at: userData.updatedAt instanceof Date ? userData.updatedAt.toISOString() : new Date(userData.updatedAt).toISOString(),
              }
              ...
              const userObject: User = {
                ...
                name: createdUser.firstName && createdUser.lastName ? `${createdUser.firstName} ${createdUser.lastName}` : createdUser.firstName || createdUser.lastName || undefined,
                ...
                subscription_status: 'inactive',
                form_limit: 5,
                created_at: createdUser.createdAt instanceof Date ? createdUser.createdAt.toISOString() : new Date(createdUser.createdAt).toISOString(),
                updated_at: createdUser.updatedAt instanceof Date ? createdUser.updatedAt.toISOString() : new Date(createdUser.updatedAt).toISOString()
              }
              setUser(userObject)
            }
          } catch (error) {
  ...
  const updateUserProfile = async (updates: Partial<User>) => {
    try {
      ...
      const frontendUser: User = {
        ...
        name: updatedUser.firstName && updatedUser.lastName ? `${updatedUser.firstName} ${updatedUser.lastName}` : updatedUser.firstName || updatedUser.lastName || undefined,
        ...
        subscription_status: updatedUser.subscriptionStatus as 'active' | 'inactive' | 'cancelled',
        stripe_customer_id: updatedUser.stripeCustomerId || undefined,
        stripe_subscription_id: undefined,
        form_limit: 5,
        created_at: typeof updatedUser.createdAt === 'string' ? updatedUser.createdAt : new Date(updatedUser.createdAt).toISOString(),
        updated_at: typeof updatedUser.updatedAt === 'string' ? updatedUser.updatedAt : new Date(updatedUser.updatedAt).toISOString(),
      }
      setUser(frontendUser)
      ...
      if (error.message.includes('Failed to update user')) {
        message = 'Could not save your changes. Please ensure all fields are valid.'
  ...
  const createAnonymousUser = async (): Promise<AnonymousUser> => {
    try {
      ...
      if (!userTracking) {
        console.log('🔄 No tracking data available, fetching tracking data first...')
        const trackingData = await userTrackingManager.trackUserVisit()
        setUserTracking({
          ip: trackingData.ip,
          fingerprint: trackingData.fingerprint,
  ...
  const refreshAuthState = async () => {
    try {
      ...
      addNotification({
        ...
        message: error?.message?.includes('NETWORK_TIMEOUT') ? 'Network timeout. Please check your connection.' : 'Failed to refresh authentication. Please try again.',
        duration: 5000
      })
      throw error
    }
  ...
  const checkUserFormLimit = async (): Promise<{ canCreate: boolean; currentCount: number; limit: number }> => {
    try {
      if (!user) {
        return { canCreate: false, currentCount: 0, limit: 5 }
      }
