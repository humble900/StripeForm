...
export const BrandKitProvider: React.FC<BrandKitProviderProps> = ({
  children,
}) => {
  ...
  useEffect(() => {
    try {
      ...
      window.dispatchEvent(
        new CustomEvent("brandKitUpdated", { detail: brandKit }),
      );
    } catch (error) {
    ...
    setBrandKit((prev) => {
      ...
      if (category === "custom") {
        ...
        if (key === "add") {
          ...
          newBrandKit.colors.custom = newBrandKit.colors.custom.filter(
            (c) => c.id !== color.id,
          );
        } else if (key === "update") {
          const index = newBrandKit.colors.custom.findIndex(
            (c) => c.id === color.id,
          );
          if (index !== -1) {
        ...
        const categoryObj =
          newBrandKit.colors[category as keyof typeof newBrandKit.colors];
    ...
    try {
      ...
      if (brandKit.colors) {
        Object.entries(brandKit.colors).forEach(([category, value]) => {
          if (typeof value === "object" && value && "hex" in value) {
            ...
            if (value.rgb)
              variables.push(`--brand-${category}-rgb: ${value.rgb};`);
            if (value.hsl)
              variables.push(`--brand-${category}-hsl: ${value.hsl};`);
            ...
            Object.entries(value).forEach(([key, color]) => {
              if (
                ...
                variables.push(
                  `--brand-${category}-${key}: ${colorValue.hex};`,
                );
                variables.push(
                  `--brand-${category}-${key}-rgb: ${colorValue.rgb};`,
                );
                variables.push(
                  `--brand-${category}-${key}-hsl: ${colorValue.hsl};`,
                );
              }
            });
          }
      ...
      if (brandKit.typography) {
        if (brandKit.typography.fontFamily) {
          Object.entries(brandKit.typography.fontFamily).forEach(
            ([key, value]) => {
        ...
        if (brandKit.typography.fontSize) {
          Object.entries(brandKit.typography.fontSize).forEach(
            ([key, value]) => {
        ...
        if (brandKit.typography.fontWeight) {
          Object.entries(brandKit.typography.fontWeight).forEach(
            ([key, value]) => {
        ...
        if (brandKit.typography.lineHeight) {
          Object.entries(brandKit.typography.lineHeight).forEach(
            ([key, value]) => {
        ...
        if (brandKit.typography.letterSpacing) {
          Object.entries(brandKit.typography.letterSpacing).forEach(
            ([key, value]) => {
      ...
      if (brandKit.formStyling) {
        if (brandKit.formStyling.layout) {
          Object.entries(brandKit.formStyling.layout).forEach(
            ([key, value]) => {
        ...
        if (brandKit.formStyling.spacing) {
          Object.entries(brandKit.formStyling.spacing).forEach(
            ([key, value]) => {
        ...
        if (brandKit.formStyling.button) {
          Object.entries(brandKit.formStyling.button).forEach(
            ([buttonType, styles]) => {
              if (styles && typeof styles === "object") {
                Object.entries(styles).forEach(([key, value]) => {
                  if (typeof value === "string" || typeof value === "number") {
                    variables.push(
                      `--form-button-${buttonType}-${key}: ${value};`,
                    );
                  }
                });
              }
        ...
        if (brandKit.formStyling.progress) {
          Object.entries(brandKit.formStyling.progress).forEach(
            ([key, value]) => {
  ...
  const applyToForm = async (formId: string) => {
    ...
    try {
      ...
      const simpleBrandKit = {
        logo:
          brandKit.logo?.current?.type === "image" && brandKit.logo?.main?.light
            ? {
                url: brandKit.logo.main.light.url,
                alt:
                  brandKit.logo.main.light.name || brandKit.logo.main.light.alt,
      ...
      window.dispatchEvent(
        new CustomEvent("applyBrandKitToForm", {
