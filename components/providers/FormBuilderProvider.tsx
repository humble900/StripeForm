...
export function FormBuilderProvider({ children }: { children: ReactNode }) {
    ...
    if (formId && formId.startsWith('form_')) {
      ...
      try {
        ...
        if (createResponse.ok) {
          ...
          console.warn('Failed to create new draft form in DB:', createResponse.status);
        }
      } catch (err) {
        ...
        const draftData = {
          ...
          draftData: {
            ...
            fields: currentFields.map((field, index) => ({
              ...
              conditionalLogic: (field as any).conditional_logic || (field as any).conditionalLogic || null
            })),
  ...
  }, [state.has_unsaved_changes, state.is_saving, state.current_form?.fields?.length, saveToMemory]) // Add saveToMemory to dependencies
