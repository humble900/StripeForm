...
export function TemplateGallery({ onClose, onTemplateSelect }: TemplateGalleryProps) {
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      case 'Business': return <BriefcaseIcon className="w-4 h-4" />
      case 'Events': return <CalendarIcon className="w-4 h-4" />
      case 'HR': return <UserGroupIcon className="w-4 h-4" />
      case 'Product': return <CubeIcon className="w-4 h-4" />
      case 'Marketing': return <MegaphoneIcon className="w-4 h-4" />
  ...
  const getCategoryGradient = (category: string) => {
    switch (category) {
      case 'Business': return 'from-blue-50 to-indigo-100'
      case 'Events': return 'from-purple-50 to-pink-100'
      case 'HR': return 'from-emerald-50 to-teal-100'
      case 'Product': return 'from-amber-50 to-orange-100'
      case 'Marketing': return 'from-rose-50 to-red-100'
          ...
          {templates.map((template) => (
            <Card key={template.id} className="group hover:shadow-lg transition-all duration-200 border-0 shadow-md">
              <CardContent className="p-0">
                {/* Template Image */}
                <div className={`relative h-48 bg-gradient-to-br ${getCategoryGradient(template.category)} rounded-t-lg overflow-hidden`}>
          ...
          {selectedTemplate && (
            <div className="space-y-6">
              ...
              <div className="bg-gray-50 rounded-lg p-6">
                <h4 className="font-semibold text-gray-900 mb-4">Form Preview</h4>
                <div className="space-y-4">
                  {selectedTemplate.templateData.fields.slice(0, 5).map((field, index) => (
                  ...
                  {selectedTemplate.templateData.fields.length > 5 && (
                    <div className="text-center text-gray-500 text-sm">
                      ... and {selectedTemplate.templateData.fields.length - 5} more questions
                    </div>
                  )}
                </div>
              </div>
