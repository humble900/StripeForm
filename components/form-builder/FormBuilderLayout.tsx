...
export function FormBuilderLayout() {
  ...
  const handleDrop = (e: React.DragEvent, targetId: string) => {
    ...
    const reorderedFields = fields.map((f) => {
      ...
      if (f.type === "end_page" || f.type === "url_redirect")
        return { ...f, order: 999 };
      qNum++;
      return { ...f, order: qNum };
    });
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <div
          className="hidden md:flex flex-col items-center py-4 px-2 gap-2 z-20 flex-shrink-0"
          style={{
            background:
              "linear-gradient(180deg, rgba(248,247,255,0.9) 0%, rgba(241,240,251,0.9) 100%)",
        ...
        >
          {NAV_ITEMS.map((item) => (
            ...
            >
              ...
              <span className="text-[9px] font-medium text-gray-500 group-hover:text-[#6C5CE7] transition-colors">
                {item.label}
              </span>
            </a>
          ))}
        ...
        <div className="flex-1 flex flex-col relative builder-canvas-bg overflow-hidden">
          ...
          <div className="lg:hidden flex items-center justify-between px-3 py-2 border-b border-gray-200/60 bg-white/60">
            <div className="flex items-center gap-1">
              <a
                href="/dashboard"
                className="p-2 text-gray-500 hover:text-[#6C5CE7] hover:bg-[#6C5CE7]/10 rounded-lg transition-colors"
                title="Dashboard"
              >
              ...
              state.current_form.fields.length > 0 && (
                <div className="hidden md:flex flex-col w-56 z-10 flex-shrink-0 h-full overflow-hidden pb-0 bg-white/40 border-r border-[#6C5CE7]/10 shadow-[4px_0_24px_-12px_rgba(108,92,231,0.15)] backdrop-blur-md">
                  ...
                  <div className="px-2 space-y-1 flex-1 overflow-y-auto sidebar-scroll pb-4 pt-1">
                    {(() => {
                      const sortedFieldsOnly = [
                        ...(state.current_form?.fields || []),
                      ].filter(
                        (f) =>
                          f.type !== "cover_slide" && f.type !== "end_page",
                      );
                      ...
                      return sorted.map((field: any) => {
                        ...
                        const isSelected =
                          state.selected_field?.id === field.id;
                        ...
                        return (
                          <div
                            ...
                            onDragStart={(e) =>
                              isDraggable && handleDragStart(e, field.id)
                            }
                            onDragOver={(e) =>
                              isDraggable && handleDragOver(e, field.id)
                            }
                          ...
                          >
                            <div className="flex items-center gap-2.5 flex-1 min-w-0">
                              ...
                              <span
                                className={`truncate flex-1 font-medium transition-colors ${isSelected ? "text-gray-900" : "text-gray-600"}`}
                              >
                                {field.label || "___"}
                              </span>
                            </div>
                  ...
                  <div
                    className="px-2 pt-2 pb-2 sticky bottom-0 mt-auto flex flex-col gap-1"
                    style={{
                      background:
                        "linear-gradient(0deg, rgba(245,243,255,0.98) 70%, transparent 100%)",
                    }}
                  >
                    <button
                      onClick={() => {
                        const existing = state.current_form?.fields?.find(
                          (f: any) => f.type === "cover_slide",
                        );
                        if (existing) {
                          selectField(existing);
                        } else {
                          addField({
                            id: `field_${Date.now()}`,
                            type: "cover_slide" as any,
                            label: "Welcome",
                            required: false,
                            order: -1,
                          } as FormField);
                    ...
                    >
                      ...
                      <span className="truncate flex-1 text-left">
                        {state.current_form?.fields?.some(
                          (f: any) => f.type === "cover_slide",
                        )
                          ? "Welcome Screen"
                          : "+ Welcome Screen"}
