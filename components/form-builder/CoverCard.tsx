...
export type CoverPreset = 'bounce' | 'typeform' | 'stitch' | 'tripe' | 'plain' | 'builder'
...
export function CoverCard({
	...
	const paddingClass = preset === 'typeform' ? 'p-6 sm:p-8 lg:px-12 xl:px-16' : preset === 'bounce' ? 'p-12' : preset === 'tripe' ? 'p-10' : 'p-10'
	...
	return (
		<div className={`relative text-center overflow-hidden border border-gray-200 ${containerClass}`} style={{ backgroundColor: bgColor }}>
			{bgUrl && <img src={bgUrl} alt="Cover background" className="absolute inset-0 w-full h-full object-cover -z-10" />}
			...
			<div className={`relative w-full max-w-4xl mx-auto ${paddingClass}`}>
				...
				<h2 className={`font-bold text-gray-900 leading-tight ${titleClass}`}>{title}</h2>
				{subtitle && (
					<p className={`text-gray-600 mt-3 sm:mt-4 lg:mt-5 xl:mt-6 2xl:mt-8 leading-relaxed ${subtitleClass}`}>{subtitle}</p>
				)}
				<div className="mt-6">
					<button
						onClick={onStart}
