      ...
      <div className="space-y-3">
        ...
        {conditional.fieldId && (
          <div className="bg-[#6C5CE7]/5 border border-[#6C5CE7]/10 rounded-xl p-3 flex gap-2">
            <svg className="w-4 h-4 text-[#6C5CE7]/60 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
            <p className="text-[11px] text-[#6C5CE7]/80 leading-relaxed">
              When <strong>"{allFields.find(f => f.id === conditional.fieldId)?.label || 'Field'}"</strong> {operators.find(o => o.value === conditional.operator)?.label} {conditional.value ? <strong>"{conditional.value}"</strong> : ''}, this question will be <strong>{conditional.action === 'show' ? 'shown' : 'hidden'}</strong>.
            </p>
          </div>
        )}
      </div>
