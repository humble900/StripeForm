...
export default function EnhancedFormPreview({
  ...
  const evaluateCondition = (conditional: any, currentData: FormData) => {
    ...
    switch (conditional.operator) {
      case 'equals': conditionMet = String(value || '') === String(targetValue || ''); break;
      case 'not_equals': conditionMet = String(value || '') !== String(targetValue || ''); break;
      case 'contains': conditionMet = String(value || '').toLowerCase().includes(String(targetValue || '').toLowerCase()); break;
      case 'greater_than': conditionMet = Number(value || 0) > Number(targetValue || 0); break;
      case 'less_than': conditionMet = Number(value || 0) < Number(targetValue || 0); break;
      case 'is_empty': conditionMet = !value || String(value).trim() === ''; break;
      case 'is_not_empty': conditionMet = !!value && String(value).trim() !== ''; break;
  ...
  const visibleFields = form.fields?.filter(field => {
    const isSpecialField = ['cover_slide', 'end_page', 'geo_restriction', 'url_redirect'].includes(field.type as string);
  ...
  const tfBg = form.theme?.background_color || (form as any)?.brandKit?.colors?.pageBackground?.hex || '#FFFFFF'
  const tfText = form.theme?.text_color || (form as any)?.brandKit?.colors?.text?.primary?.hex || '#1F2937'
  const tfPrimary = form.theme?.primary_color || (form as any)?.brandKit?.colors?.buttonPrimary?.hex || '#6C5CE7'
  const tfFont = (form as any)?.brandKit?.typography?.fontFamily?.primary || form.theme?.font_family || 'Inter, sans-serif'

  // If showing thank you page
  if (showThankYou) {
    const endPageField = form.fields?.find(f => f.type === 'end_page') as any
    const tyTitle = endPageField?.settings?.endPageTitle || form.settings?.success_message || 'Thank you!'
    const tyMsg = endPageField?.settings?.endPageMessage || 'Your response has been submitted successfully.'
    ...
    if (isProgressive) {
      return (
        <div className="tf-thankyou" style={{ backgroundColor: tyBg, color: tfText, fontFamily: tfFont }}>
          <div className="tf-thankyou-content">
            {/* Animated check icon */}
            <div className="tf-thankyou-icon tf-scale-in" style={{ backgroundColor: `${tfPrimary}18` }}>
              <svg width="32" height="32" viewBox="0 0 24 24" fill="none">
                <path className="tf-check-anim" d="M5 13l4 4L19 7" stroke={tfPrimary} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </div>
            <h1 className="tf-thankyou-title" style={{ color: tfText }}>{tyTitle}</h1>
            <p className="tf-thankyou-desc" style={{ color: tfText }}>{tyMsg}</p>

            {tyBtnText && (
              <a
                href={tyBtnUrl || '#'}
                onClick={(e) => { if (!tyBtnUrl) { e.preventDefault(); onClose?.() } }}
            ...
            <div className="tf-share-row">
              <button className="tf-share-btn" title="Copy link" onClick={() => navigator.clipboard?.writeText(window.location.href)}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71" /><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71" /></svg>
              </button>
              <button className="tf-share-btn" title="Share on X" onClick={() => window.open(`https://x.com/intent/tweet?url=${encodeURIComponent(window.location.href)}`, '_blank')}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" /></svg>
              </button>
              <button className="tf-share-btn" title="Share on LinkedIn" onClick={() => window.open(`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(window.location.href)}`, '_blank')}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" /></svg>
              </button>
            </div>

            {isPreview && (
              <button onClick={handleReset} className="mt-4 text-sm opacity-50 hover:opacity-80 underline" style={{ color: tfText }}>
                Restart form
              </button>
            )}
          </div>
    ...
    if (endPageField) {
      const hasBgImage = endPageField.settings?.coverBackgroundImage || form.theme?.header_image_url
      const bgColor = endPageField.settings?.coverBackgroundColor || tfBg
      const textColor = hasBgImage ? '#FFFFFF' : tfText

      return (
        <div className="min-h-screen flex items-center justify-center relative overflow-hidden" style={{ backgroundColor: form.theme?.gallery_theme_id ? 'transparent' : bgColor, fontFamily: tfFont }}>
          <ThemeArtBackground themeId={form.theme?.gallery_theme_id} backgroundColor={bgColor} />

          {hasBgImage && (
            <>
              <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${hasBgImage})` }} />
          ...
          <div className="relative z-10 w-full max-w-4xl mx-auto px-6 py-12 flex flex-col items-center justify-center text-center animate-in fade-in slide-in-from-bottom-8 duration-700">
            ...
            <h1 className="text-5xl md:text-6xl lg:text-7xl font-bold mb-6 tracking-tight leading-tight" style={{ color: textColor }}>
              {tyTitle}
            </h1>

            <p className="text-xl md:text-2xl mb-12 max-w-2xl text-center leading-relaxed" style={{ color: textColor, opacity: 0.8 }}>
              {tyMsg}
            </p>
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              {isPreview && (
                <button onClick={handleReset} className="mt-6 text-base font-medium opacity-60 hover:opacity-100 transition-opacity flex items-center gap-2" style={{ color: textColor }}>
  ...
  if (coverField && !hasStartedForm && isProgressive) {
    const hasCoverBgImage = coverField.settings?.coverBackgroundImage || form.theme?.header_image_url
    ...
    if (isProgressive) {
      ...
      return (
        <div className="tf-cover relative" style={{ backgroundColor: form.theme?.gallery_theme_id ? 'transparent' : coverBgColor, fontFamily: tfFont }}>
          <ThemeArtBackground themeId={form.theme?.gallery_theme_id} backgroundColor={coverBgColor} />
          {hasCoverBgImage && (
            <>
              <div className="tf-cover-bg" style={{ backgroundImage: `url(${hasCoverBgImage})` }} />
              <div className="tf-cover-overlay" />
            </>
          )}
          <div className="tf-cover-content">
            {form.brandKit?.logo?.url && (
              <img src={form.brandKit.logo.url} alt={form.brandKit.logo.alt || 'Logo'} style={{ maxHeight: 56, maxWidth: 180, objectFit: 'contain' }} />
            )}
            ...
            <div className="tf-action-row" style={{ justifyContent: 'center' }}>
              <button
                ...
                className="tf-ok-btn"
                style={{ backgroundColor: coverField.settings?.coverButtonColor || tfPrimary, fontSize: '1.0625rem', padding: '0.75rem 2rem', borderRadius: '999px' }}
              >
                {coverField.settings?.coverCtaText || 'Start'}
                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" width="18" height="18"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6" /></svg>
              </button>
              <span className="tf-hint" style={{ color: coverTextColor }}>press <kbd>Enter</kbd> ↵</span>
            </div>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center relative overflow-hidden" style={{ backgroundColor: form.theme?.gallery_theme_id ? 'transparent' : coverBgColor, fontFamily: tfFont }}>
        <ThemeArtBackground themeId={form.theme?.gallery_theme_id} backgroundColor={coverBgColor} />

        {hasCoverBgImage && (
          <>
            <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${hasCoverBgImage})` }} />
        ...
        {!hasCoverBgImage && !form.theme?.gallery_theme_id && (
          <div className="absolute inset-0 opacity-5">
            <div className="absolute inset-0" style={{ backgroundImage: `radial-gradient(circle at 25% 25%, ${tfPrimary} 0%, transparent 50%), radial-gradient(circle at 75% 75%, #8b5cf6 0%, transparent 50%)` }} />
          </div>
        )}
        ...
        <div className="relative z-10 w-full max-w-5xl mx-auto px-6 py-12 flex flex-col items-center justify-center text-center animate-in fade-in slide-in-from-bottom-8 duration-700">
          {form.brandKit?.logo?.url && (
            <div className="flex justify-center mb-10">
              <img src={form.brandKit.logo.url} alt={form.brandKit.logo.alt || 'Logo'} className="max-w-32 max-h-24 object-contain shadow-2xl rounded-xl" />
            </div>
          )}

          <div className="space-y-6 mb-12">
            <h1 className="text-5xl md:text-6xl lg:text-7xl font-extrabold tracking-tight leading-tight drop-shadow-sm" style={{ color: coverTextColor }}>
              {coverField.settings?.coverTitle || form.title || 'Welcome'}
            </h1>

            {coverField.settings?.coverSubtitle && (
              <p className="text-xl md:text-2xl lg:text-3xl max-w-3xl mx-auto leading-relaxed font-medium drop-shadow-sm" style={{ color: coverTextColor, opacity: 0.85 }}>
                {coverField.settings.coverSubtitle}
              </p>
            )}
          </div>
          ...
          <div className="flex flex-col items-center gap-4">
            <button
              ...
              style={{ background: coverField.settings?.coverButtonColor || tfPrimary }}
            >
              <span className="relative z-10 flex items-center gap-3">
                {coverField.settings?.coverCtaText || 'Get Started'}
                <svg className="w-6 h-6 group-hover:translate-x-1.5 transition-transform" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M13 5l7 7-7 7M5 5l7 7-7 7" /></svg>
              </span>
              <div className="absolute inset-0 h-full w-full border-2 border-white/20 rounded-lg group-hover:border-white/40 transition-colors" />
            </button>
            <div className="text-sm font-medium mt-3 flex items-center gap-2 opacity-70" style={{ color: coverTextColor }}>
  ...
  if (isProgressive) {
    ...
    return (
      <div className="tf-viewport relative" style={{ backgroundColor: form.theme?.gallery_theme_id ? 'transparent' : tfBg, fontFamily: tfFont, color: tfText }}>
        {/* Theme art SVG background */}
        <ThemeArtBackground themeId={form.theme?.gallery_theme_id} backgroundColor={tfBg} />
        {/* Inject custom CSS */}
        {isPreview && form.theme?.custom_css && (
          <style dangerouslySetInnerHTML={{ __html: String(form.theme.custom_css) }} />
        )}

        {/* Preview banner */}
        {isPreview && (
          <div className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 py-1.5" style={{ background: `${tfPrimary}18`, backdropFilter: 'blur(8px)' }}>
            <Badge variant="secondary" className="text-xs">Preview</Badge>
            <div className="flex items-center gap-2">
              <Button variant="outline" className="h-6 px-2 text-xs" onClick={handleReset}>Reset</Button>
              {onClose && <Button variant="outline" className="h-6 px-2 text-xs" onClick={onClose}>Close</Button>}
            </div>
          </div>
        )}
        ...
        {visibleFields.map((field, idx) => {
          const slideClass = idx === currentStep
            ? 'tf-slide tf-active'
            : idx < currentStep
              ? 'tf-slide tf-exit-up'
            ...
            <div key={field.id} className={slideClass}>
              <div className="tf-slide-content">
                ...
                <h2 className="tf-q-heading" style={{ color: tfText }}>
                  ...
                  {field.required && <span style={{ color: tfPrimary }}> *</span>}
                </h2>

                {/* Description */}
                {field.description && (
                  <p className="tf-q-desc" style={{ color: tfText }}>{field.description}</p>
                )}
                ...
                <div className="tf-action-row">
                  ...
                  >
                    {isSubmitting ? (
                      <><span className="inline-block w-4 h-4 border-2 border-white/60 border-t-transparent rounded-full animate-spin" /> Submitting...</>
                    ) : isLast ? (
                      <>{(form as any)?.settings?.submit_button_text || 'Submit'} <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg></>
                    ) : (
                      <>OK <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg></>
                    )}
                  </button>
        ...
        <div className="tf-nav-widget">
          <button className="tf-nav-btn hover:opacity-90" onClick={goPrev} disabled={currentStep === 0} aria-label="Previous" style={{ backgroundColor: tfPrimary, color: (form.theme as any)?.button_text_color || '#FFFFFF', borderColor: tfPrimary }}>
            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 15l7-7 7 7" /></svg>
          </button>
          <button className="tf-nav-btn hover:opacity-90" onClick={goNext} disabled={currentStep >= totalSteps - 1} aria-label="Next" style={{ backgroundColor: tfPrimary, color: (form.theme as any)?.button_text_color || '#FFFFFF', borderColor: tfPrimary }}>
        ...
        <div className="tf-progress-bar">
          <div className="tf-progress-fill" style={{ width: `${progressPercent}%`, backgroundColor: tfPrimary }} />
        </div>
      </div>
    )
  }
  ...
  const getCanvasBgColor = () => {
    ...
    return form.theme?.background_color || (() => {
      const cover = form.fields?.find((f: any) => f.type === 'cover_slide') as any
      return cover?.settings?.coverBackgroundColor || '#f8fafc'
    })()
  }

  return (
    <div className={`w-full min-h-screen flex justify-center py-6 px-3 sm:px-6 md:px-8 ${(isPreview) ? 'bg-transparent items-start' : 'bg-gray-50/50 items-center'}`} style={{ fontFamily: tfFont }}>
      ...
      <div
        ...
        style={{
          ...
          backgroundImage: ((form as any)?.brandKit?.backgroundImageUrl) ? `url(${(form as any).brandKit.backgroundImageUrl})` : undefined,
          backgroundSize: ((form as any)?.brandKit?.backgroundImageUrl) ? 'cover' : undefined,
          backgroundPosition: ((form as any)?.brandKit?.backgroundImageUrl) ? 'center' : undefined,
          minHeight: 'calc(100vh - 160px)',
        }}
      >
        {/* Theme art SVG background */}
        <ThemeArtBackground themeId={form.theme?.gallery_theme_id} backgroundColor={undefined} />
        ...
        {isPreview && (
          <div className="relative z-50 flex items-center justify-between px-4 py-2 bg-[#6C5CE7]/5 border-b border-[#6C5CE7]/10 backdrop-blur-sm">
            <Badge variant="outline" className="text-[#6C5CE7] border-[#6C5CE7]/20 bg-white/50 shadow-sm text-[10px] uppercase font-bold tracking-wider">Preview Mode</Badge>
            <div className="flex items-center gap-2">
              <Button variant="ghost" className="h-7 px-3 text-xs text-gray-500 hover:text-gray-900 bg-white/50 border border-transparent shadow-sm hover:border-gray-200 transition-all rounded-lg" onClick={handleReset}>Reset</Button>
              {onClose && <Button variant="ghost" className="h-7 px-3 text-xs text-gray-500 hover:text-gray-900 bg-white/50 border border-transparent shadow-sm hover:border-gray-200 transition-all rounded-lg" onClick={onClose}>Close</Button>}
            </div>
          </div>
        )}
        ...
        <div className="flex-1 w-full max-w-[660px] mx-auto py-12 px-6 sm:px-8 flex flex-col relative z-20">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* Header Section */}
            {(form.brandKit?.logo?.url || (form.theme as any)?.logo?.url || (form.theme as any)?.textLogo?.text || (!coverField && (form.title || form.description)) || form.theme?.header_image_url) && (
              <div
                className="relative w-full py-8 px-6 rounded-2xl overflow-hidden mb-8 shadow-sm border border-gray-100/50"
                style={{
                  backgroundColor: ((form as any)?.brandKit?.colors?.headerBackground?.hex || form.theme?.header_color || 'rgba(255, 255, 255, 0.7)') as string,
                  backdropFilter: (!form.theme?.header_color) ? 'blur(12px)' : 'none',
              ...
              >
                {form.theme?.header_image_url && (
                  <div className="absolute inset-0" style={{ backgroundColor: form.theme?.header_color ? `${form.theme.header_color}CC` : 'rgba(0,0,0,0.4)' }} />
                )}
                    ...
                    <div className="w-full flex items-center justify-center mb-4">
                      {(form.brandKit?.logo?.url || (form.theme as any)?.logo?.url) ? (
                        <img
                          ...
                          className="object-contain" style={{ maxHeight: '64px', maxWidth: '220px' }}
                        />
                      ) : (
                        <h1 className="font-bold" style={{
                          color: form.theme?.header_image_url ? '#FFFFFF' : ((form.brandKit as any)?.textLogo?.color || '#111827'),
                          fontFamily: ((form.brandKit as any)?.textLogo?.fontFamily || 'inherit'),
                          fontWeight: ((form.brandKit as any)?.textLogo?.fontWeight || 700) as any,
                          fontSize: '1.25rem'
                        }}>
                    ...
                    <div className="space-y-4 max-w-2xl">
                      {!coverField && form.title && (
                        <h1 className="text-3xl md:text-4xl font-bold" style={{
                          color: form.theme?.header_image_url ? '#FFFFFF' : (tfText),
                          fontFamily: tfFont,
                      ...
                      {!coverField && form.description && (
                        <p className="text-lg md:text-xl" style={{
                          color: form.theme?.header_image_url ? '#F3F4F6' : tfText, opacity: 0.7,
                          fontFamily: tfFont,
                          textShadow: form.theme?.header_image_url ? '0 2px 4px rgba(0,0,0,0.5)' : 'none'
                        }}>{form.description}</p>
                      )}
            ...
            {!isProgressive && coverField && (
              <div className="mb-12 text-center space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-700">
                <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight" style={{ color: tfText }}>
                  {coverField.settings?.coverTitle || form.title || 'Welcome'}
                </h1>
                {coverField.settings?.coverSubtitle && (
                  <p className="text-xl opacity-80 max-w-2xl mx-auto" style={{ color: tfText }}>
                    {coverField.settings.coverSubtitle}
                  </p>
                )}
              </div>
            ...
            <div className="relative space-y-8">
              {currentFields.map((field) => (
                <div key={field.id} className="relative group py-6 transition-all duration-300">
                  <h2 className="text-2xl font-bold mb-4 leading-tight" style={{ color: tfText }}>
                    {field.label || 'Untitled question'}
                    {field.required && <span className="text-red-500 ml-1.5">*</span>}
                  </h2>
                  {field.description && (
                    <p className="text-lg opacity-80 mb-5 leading-relaxed" style={{ color: tfText }}>
                      {field.description}
                    </p>
                  )}
    ...
    <div className="space-y-4 p-6 border rounded-lg bg-gray-50">
      <div className="text-center">
        ...
        <div className="mt-4 text-xs text-gray-500 flex items-center justify-center gap-1.5">
          <LightBulbIcon className="w-4 h-4 text-yellow-500" /> Enable Stripe integration in form settings to collect real payments
        </div>
      </div>
    </div>
  )
