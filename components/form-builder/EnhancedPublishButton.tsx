...
export function EnhancedPublishButton({
  ...
  const handleRepublish = async () => {
    ...
    try {
      ...
      addNotification({
        ...
        title: "Form Updated",
        message:
          "Your published form has been updated with the latest changes!",
        duration: 5000,
      });
    } catch (error) {
  ...
  const copyToClipboard = async (text: string) => {
    try {
      ...
      addNotification({
        ...
        title: "Copy Failed",
        message:
          "Failed to copy link to clipboard. Your browser might be blocking it or you need to grant permission.",
        duration: 3000,
      });
    }
  };
  ...
  const shareOptions: ShareOption[] = [
    ...
    },
    {
      ...
      action: () => {
        const subject = encodeURIComponent(
          `Check out my form: ${state.current_form?.title}`,
        );
        const body = encodeURIComponent(
          `Hi! I'd like to share my form with you: ${getEffectiveShareUrl()}`,
        );
    ...
    },
    {
      ...
      action: () => {
        const text = encodeURIComponent(
          state.current_form?.title || "Check out this form",
        );
    ...
    },
    {
      ...
      action: () => {
        const text = encodeURIComponent(
          `${state.current_form?.title || "Form"} - ${getEffectiveShareUrl()}`,
        );
