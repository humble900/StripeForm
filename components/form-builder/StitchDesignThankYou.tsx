    ...
    <div className="relative flex size-full min-h-screen flex-col justify-between group/design-root overflow-x-hidden" style={{ backgroundColor, fontFamily }}>
      <div>
        <div className="flex items-center p-4 pb-2 justify-between" style={{ backgroundColor }}>
          ...
          <h2 className="text-lg font-bold leading-tight tracking-[-0.015em] flex-1 text-center pr-12" style={{ color: textColor }}>{formTitle}</h2>
        </div>
        <h2 className="tracking-light text-[clamp(18px,4vw,28px)] font-bold leading-tight px-4 text-center pb-3 pt-5" style={{ color: textColor }}>{thankYouTitle}</h2>
        <p className="text-[clamp(13px,2.5vw,16px)] font-normal leading-normal pb-3 pt-1 px-4 text-center" style={{ color: textColor }}>{thankYouDescription}</p>
      </div>
      <div>
