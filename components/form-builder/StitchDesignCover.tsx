...
export default function StitchDesignCover({ 
  ...
  return (
    <div className="relative flex size-full min-h-screen flex-col justify-between group/design-root overflow-x-hidden" style={{ backgroundColor, fontFamily }}>
      <div>
        <h2 className="tracking-light text-[clamp(18px,4vw,28px)] font-bold leading-tight px-4 text-center pb-3 pt-8" style={{ color: textColor }}>{formTitle}</h2>
        {formDescription && (
          <p className="text-[clamp(13px,2.5vw,16px)] font-normal leading-normal pb-3 pt-1 px-4 text-center" style={{ color: textColor }}>{formDescription}</p>
        )}
      </div>
      <div>
