...
export function EnhancedAutoSave({ 
  ...
  const saveDraftToServer = useCallback(async (formData: Form) => {
  ...
  }, [defaultConfig.enableServerBackup, isAuthenticated, user, isAnonymous, getUserTrackingData, onDraftSaved])
  ...
  }, [form, state.isSaving, state.isOnline, state.retryCount, saveToLocalStorage, saveDraftToServer, defaultConfig.maxRetries])
  ...
  }, [form.id, state.isOnline, defaultConfig.enableServerBackup, isAuthenticated, user, isAnonymous, getUserTrackingData, loadFromLocalStorage, onDraftRestored])
  ...
  const getStatusText = () => {
    switch (state.saveStatus) {
      ...
      default:
        return state.lastSaved ? `Last saved ${formatTimeAgo(state.lastSaved)}` : 'Not saved yet'
    }
  }
