  ...
  useEffect(() => {
    const fetchForms = async () => {
      if (!isAuthenticated && !isAnonymous) {
        console.log(
          "📊 Analytics: Not authenticated and not anonymous, skipping form fetch",
        );
        return;
      }
      ...
      try {
        ...
        if (isAuthenticated && user) {
          ...
          console.log(
            "📊 Analytics: Getting fingerprint for anonymous user...",
          );
          try {
            ...
            console.error(
              "📊 Analytics: Error getting fingerprint:",
              fingerprintError,
            );
        ...
        try {
          ...
          const convertedForms = (userForms || []).map((form: any) => ({
            ...
            user_id: form.userId,
            is_published:
              form.status === "published" || form.isPublished === true,
            published_url:
              form.publishedUrl ||
              (form.status === "published" ? `/forms/${form.slug}` : undefined),
    ...
    const fetchResponses = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/user/forms/${selectedForm}/responses`,
          {
            headers: {
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="flex gap-2">
            ...
            >
              <div
                className={`w-4 h-4 ${isLoadingResponses ? "animate-spin" : ""}`}
              >
                {isLoadingResponses ? "⟳" : "⟳"}
              </div>
              Refresh
        ...
        {!selectedForm ? (
          <Card>
            <CardContent className="text-center py-12">
              ...
              <p className="text-gray-600 mb-6">
                Choose a form from the dropdown above to view detailed analytics
                and insights.
              </p>
              {forms.length > 0 && (
          ...
          <Card>
            <CardContent className="text-center py-12">
              ...
              <p className="text-gray-600">
                The selected form could not be found. Please select another
                form.
              </p>
            </CardContent>
          </Card>
