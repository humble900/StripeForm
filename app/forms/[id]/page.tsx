...
export default function PublishedFormPage() {
  ...
  const AUTOSAVE_RESP_KEY = formId
    ? `stripeform-autosave-response-${formId}`
    : "";
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 404) {
            setError(
              "Form not found. It may have been deleted or the URL is incorrect.",
            );
            return;
          }
          throw new Error(
        ...
        if (
          !canPreview &&
          (!formData.isPublished || formData.status !== "published")
        ) {
        ...
        const hasCover = formData.fields?.some(
          (f: any) => f.type === "cover_slide",
        );
        setHasStarted(
          hasCover ? false : formData.settings?.use_cover ? false : true,
        );
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const applyGeo = async () => {
      const geoField = form.fields?.find(
        (f: any) => f.type === "geo_restriction",
      ) as any;
      if (!geoField) return;
      ...
      try {
        ...
        const countryList: string[] =
          (geoField.settings?.geoAllowedCountries as any) || [];
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      try {
        localStorage.removeItem(AUTOSAVE_RESP_KEY);
      } catch {}
    } catch (error) {
  ...
  const getPageBackground = (): string => {
    ...
    return (
      brand.pageBackground?.hex || form?.theme?.background_color || "#F9FAFB"
    );
  };
  ...
  const getFieldRadius = (field?: any): number => {
    ...
    return typeof fieldRadius === "number"
      ? fieldRadius
      ...
      >
        <div className="text-center">
          <div
            ...
            style={{ borderColor: form?.theme?.primary_color || "#3B82F6" }}
          ></div>
          <p
            className="mt-2"
            style={{ color: form?.theme?.text_color || "#6B7280" }}
          >
            Loading form...
          </p>
        </div>
      ...
      >
        <div className="text-center">
          <h1
            ...
            style={{ color: form?.theme?.text_color || "#1F2937" }}
          >
            Form Not Found
          </h1>
          <p style={{ color: form?.theme?.text_color || "#6B7280" }}>
            {error ||
