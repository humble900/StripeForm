    ...
    withErrorHandling(async (request: NextRequest) => {
      ...
      if (formUpdates.status === "published" && !existingForm.publishedAt) {
        ...
        if (!isAdmin && !isPro) {
          ...
          const userPublished = (userForms || []).filter(
            (f: any) => f.userId === ownerId && f.status === "published",
          ).length;
          if (userPublished >= 5) {
            return NextResponse.json(
              {
                success: false,
                message:
                  "Publish limit reached for your plan. Please upgrade to Pro to publish more forms.",
              },
              { status: 403 },
            );
          }
      ...
      const slugIsStale =
        currentSlug.startsWith("untitled-form") &&
        currentTitle.toLowerCase() !== "untitled form";

      if (
        (isPublishing || formUpdates.title) &&
        slugIsStale &&
        currentTitle.trim()
      ) {
        ...
        console.log(
          `🔄 Slug regenerated: "${currentSlug}" → "${formUpdates.slug}"`,
        );
      }
      ...
      if (formWithFields.status === "published" && !publishedUrl) {
        ...
        const baseUrl =
          process.env.NEXT_PUBLIC_APP_URL || "https://stripeform.com";
