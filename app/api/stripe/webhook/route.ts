  ...
  try {
    ...
    switch (event.type) {
      ...
      case "setup_intent.setup_failed":
        await handleSetupIntentSetupFailed(
          event.data.object as Stripe.SetupIntent,
        );
        break;
...
async function handlePaymentIntentSucceeded(
  ...
  try {
    await db.updatePaymentIntent(paymentIntent.id, { status: "succeeded" });
...
async function handlePaymentIntentFailed(paymentIntent: Stripe.PaymentIntent) {
  try {
    await db.updatePaymentIntent(paymentIntent.id, { status: "failed" });
...
async function handleSubscriptionCreated(subscription: Stripe.Subscription) {
  try {
    ...
    if (subscription.metadata?.user_id) {
      ...
      console.log(
        `Subscription created for user ${subscription.metadata.user_id}: ${subscriptionTier}`,
      );
    }
  } catch (error) {
...
async function handleSubscriptionUpdated(subscription: Stripe.Subscription) {
  try {
    ...
    if (subscription.metadata?.user_id) {
      ...
      console.log(
        `Subscription updated for user ${subscription.metadata.user_id}: ${subscriptionTier}`,
      );
    }
  } catch (error) {
...
async function handleSubscriptionDeleted(subscription: Stripe.Subscription) {
  try {
    ...
    if (subscription.metadata?.user_id) {
      ...
      console.log(
        `Subscription deleted for user ${subscription.metadata.user_id}`,
      );
    }
  } catch (error) {
...
async function handlePaymentIntentCanceled(
  ...
  try {
    await db.updatePaymentIntent(paymentIntent.id, { status: "canceled" });
...
async function handleChargeDisputeUpdated(dispute: Stripe.Dispute) {
  try {
    console.log(
      `Dispute updated for charge ${dispute.charge}: ${dispute.status}`,
    );
...
async function handleChargeDisputeClosed(dispute: Stripe.Dispute) {
  try {
    console.log(
      `Dispute closed for charge ${dispute.charge}: ${dispute.status}`,
    );
...
async function handleSubscriptionTrialWillEnd(
  ...
  try {
    console.log(
      `Subscription trial will end for customer ${subscription.customer}`,
    );
...
async function handleInvoiceCreated(invoice: Stripe.Invoice) {
  try {
    console.log(
      `Invoice created: ${invoice.id} for customer ${invoice.customer}`,
    );
