...
export async function POST(request: NextRequest) {
  try {
    ...
    let idToUse =
      incomingId || (plan === "pro_yearly" ? fallbackYearly : fallbackMonthly);
    if (!idToUse) {
      return NextResponse.json(
        {
          error:
            "Stripe price/product ID not configured. Provide priceId or set STRIPE_PRO_MONTHLY_PRICE_ID / NEXT_PUBLIC_STRIPE_PRO_MONTHLY_PRICE_ID.",
        },
        { status: 400 },
      );
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url:
        successUrl ||
        `${origin}/dashboard?upgrade=success&session_id={CHECKOUT_SESSION_ID}`,
