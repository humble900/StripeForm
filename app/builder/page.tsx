...
function FormBuilderContent() {
  ...
  const { state, dispatch, restoreFromLocalStorage, loadFromMemory } =
    useFormBuilder();
    ...
    const initializeBuilder = async () => {
      try {
        ...
        if (formIdFromQuery && state.current_form?.id !== formIdFromQuery) {
          ...
          if (restored) {
            console.log(
              "✅ Form restored from localStorage, will sync with server",
            );
            return;
          }
            ...
            if (formData.success && formData.data) {
              newForm = {
                ...
                fields: (formData.data.fields || []).map((fld: any) => ({
                  ...
                  conditional_logic:
                    fld.conditionalLogic || fld.conditional_logic || null,
                })),
        ...
        if (templateId) {
          console.warn(
            "⚠️ Direct template loading detected - this should not happen anymore",
          );
          addNotification({
            type: "warning",
            title: "Template Loading",
            message:
              'Please use the "Use Template" button to copy templates properly.',
            duration: 5000,
          });
        ...
        if (!state.current_form || state.current_form.id === "default-form") {
          ...
          if (memoryForms.length > 0) {
            ...
            if (restored) {
              console.log(
                "✅ Most recent form restored from memory:",
                mostRecentForm.id,
              );
              return;
            }
          ...
          console.log(
            "🎯 New form created for builder (memory only):",
            newForm,
          );
          dispatch({ type: "SET_CURRENT_FORM", payload: newForm });
          console.log(
            "✅ Form builder initialization complete (memory storage)",
          );
        } else {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <InlineLoading
            size="lg"
            text="Initializing form builder..."
            variant="dots"
          />
        </div>
      </div>
