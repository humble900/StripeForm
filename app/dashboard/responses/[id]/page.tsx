...
const FormResponsesPage = () => {
  ...
  const [fields, setFields] = useState<
    Array<{ id: string; title?: string; label?: string; required?: boolean }>
  >([]);
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-5 h-5 bg-blue-100 rounded-full flex items-center justify-center text-[10px] font-medium text-blue-700">
              {index + 1}
            </div>
            <div>
              <p className="text-sm font-medium text-gray-900 truncate max-w-[50vw]">
                {label}
              </p>
              <p className="text-[10px] text-gray-500">
                {formatDate(response.submittedAt)} • {answerCount} fields
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <span
              className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${isComplete ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"}`}
            >
